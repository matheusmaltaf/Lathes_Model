...
try:
    ...
    uf.tsfresh_NaN_filter(input_id)
...
for it,(kf_train_idx, kf_test_idx) in enumerate(rskf.split(unique_id,rskf_target)):
    ...
    SelectedFeatures = uf.tsfresh_chucksize(train_set,input_id)

    with open('Classification/relevance_table__' + input_id + '__' + str(it) + '__.pkl', 'wb') as f:
    ...
    SODA_parameters = uf.SODA(ReducedFeatures,min_gra,max_gra,1) 
    ...
    Classifiers_result = uf.non_parametric_classification(ReducedFeatures['ReducedFeatures'],projected_features,
                        GA_parameters,test_set_target, delta)

    with open('Classification/Classifiers_result__' + input_id + '__' + str(it) + '__.pkl', 'wb') as f:
    ...
    fig.savefig('PCA_Analyses/Figures/PCA_visualization__' + input_id + '__' + str(it) + '__.png', bbox_inches='tight')
