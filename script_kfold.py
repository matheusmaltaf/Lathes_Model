...
new_n_measures = int(str(sys.argv[1]))

default_fc_parameters = {
    "fft_coefficient": [{"coeff": k, "attr": a} for a, k in product(["real", "imag", "abs", "angle"], range(new_n_measures // 2))]
}
...
class LathesModel(object):
    ...
    def plot_variation_held(
        self, PATH=False, figsize=[16, 8], title_fontsize=22, y_fontsize=27, x_fontsize=20, y_ticks_fontsize=22, x_ticks_fontsize=22, show=False
    ):
    ...
    def plot_features_contribution(
        self, PATH=False, figsize=[16, 8], title_fontsize=16, y_fontsize=20, x_fontsize=20, y_ticks_fontsize=16, x_ticks_fontsize=18, show=False
    ):
        ...
        try:
            ...
            fig.suptitle("Features Weighted Contribution Percentage", fontsize=title_fontsize)
            ax = fig.subplots(1, 1)
            s = dict(sorted(self.features_contribution_.items(), key=lambda item: item[1], reverse=True))
    ...
    def plot_best_features_contribution(
        self,
        best=20,
        PATH=False,
        figsize=[16, 8],
        ...
        try:
            ...
            fig.suptitle("Best Features Weighted Contribution Percentage", fontsize=title_fontsize)
    ...
    def plot_scatter_features(
        self, PATH=False, figsize=[14, 10], size=50, label_fontsize=20, label_pad=18, ticks_fontsize=16, cmap="viridis", show=False
    ):
        ...
        try:
            ...
            legend_elements = [
                Line2D([0], [0], marker="o", color="tab:blue", linestyle="None", markersize=15, label="Adequate Tools"),
                Line2D([0], [0], marker="o", color="tab:red", linestyle="None", markersize=15, label="Inadequate Tools"),
            ]
    ...
    def plot_soda(self, PATH=False, figsize=[14, 10], s=50, label_fontsize=20, label_pad=18, ticks_fontsize=16, cmap="viridis", show=False):
        """Plot dataset projected on first 2 or 3 PCs divided by SODA
    ...
    def plot_GA(self, PATH=False, figsize=[14, 10], s=50, label_fontsize=20, label_pad=18, ticks_fontsize=16, cmap="viridis", show=False):
...
if __name__ == "__main__":
    ...
    for input_id in input_list:
        ...
        for it, (train_unique_index, test_unique_index) in enumerate(kf.split(unique_id, target)):
            ...
            train_index = np.zeros(L_train * new_n_measures, dtype=np.int32)
            for ii in range(L_train):
                train_index[ii * new_n_measures : (ii + 1) * new_n_measures] = list(
                    range(train_unique_index[ii] * n_measures, (train_unique_index[ii] + 1) * n_measures - (n_measures - new_n_measures))
                )
            L_test = test_unique_index.shape[0]
            test_index = np.zeros(L_test * new_n_measures, dtype=np.int32)
            for ii in range(L_test):
                test_index[ii * new_n_measures : (ii + 1) * new_n_measures] = list(
                    range(test_unique_index[ii] * n_measures, (test_unique_index[ii] + 1) * n_measures - (n_measures - new_n_measures))
                )
            ...
            y_test = y_test[::new_n_measures]
            Classifiers_result = {}

            print("-----------------------------")
            print("n_time_series", n_timeseries)
            print("n_measures:", n_measures)
            print("new_n_measures:", new_n_measures)
            print("-----------------------------")
            ...
            for st in select_list:
                model = LathesModel(N_PCs=7, n_jobs=0)
                for N_PCs in [5, 7, 10, 15]:
                    Classifiers_result[N_PCs] = {}
                    for gra in gra_list:
                        Classifiers_result[N_PCs][gra] = {
                            "Nearest Neighbors": {},
                            "Linear SVM": {},
                            "RBF SVM": {},
                            "Gaussian Process": {},
                        ...
                        for name, clf in zip(names, classifiers):
                            Classifiers_result[N_PCs][gra][name] = {"Accuracy": 0, "time": 0}
                            ...
                            if model.one_class_:
                                Classifiers_result[N_PCs][gra] = "Error: Only one class"
                                break
                            else:
                                y_pred = model.predict_after_tsfresh(X_test)
                                acc = balanced_accuracy_score(y_test, y_pred)

                                Classifiers_result[N_PCs][gra][name]["Accuracy"] = acc
                                Classifiers_result[N_PCs][gra][name]["time"] = model.predict_time_
                                Classifiers_result[N_PCs][gra][name]["features"] = model.X_selected_.shape[1]
                                Classifiers_result[N_PCs][gra][name]["variation"] = np.sum(model.variation_kept_)
                                Classifiers_result[N_PCs][gra][name]["matrix"] = confusion_matrix(y_test, y_pred)
                                Classifiers_result[N_PCs][gra][name]["y_test"] = y_test
                                Classifiers_result[N_PCs][gra][name]["y_pred"] = y_pred

                    with open(
                        "Classification_{}_tudo_filtrado/Classifiers_result__{}__{}__{}__.pkl".format(new_n_measures, input_id, it, st), "wb"
                    ) as f:
